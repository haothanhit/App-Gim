...
import kotlinx.android.synthetic.main.fragment_rock_hard.*

class RockHardFragment : BaseFragment() {
    ...
    override fun onCreateView(
        ...
        return inflater.inflate(R.layout.fragment_rock_hard, container, false)
    ...
    private fun initView() {
        var arr = DatabaseAccess.getInstance(context!!).getALLDayRock
        rcv_Main_RockHard.layoutManager =
            LinearLayoutManager(context, LinearLayout.VERTICAL, false)
        rcv_Main_RockHard.adapter = DayAdapter(arr, { dayDB: DayDB -> partItemClicked(dayDB) })
        rcv_Main_RockHard.setHasFixedSize(true)
    ...
    override fun onResume() {
        super.onResume()
        if (rcv_Main_RockHard != null && userVisibleHint) {
            var arr = DatabaseAccess.getInstance(context!!).getALLDayRock
            rcv_Main_RockHard.adapter = DayAdapter(arr, { dayDB: DayDB -> partItemClicked(dayDB) })
        }
